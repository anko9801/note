...
\begin{proof}
  ...
  時間平均を取ると次のようになる。
  % \begin{align}
  %   \braket<\LL_{spin}> & = \braket<\frac{1}{4\pi c}\int\frac{\dl{\kk}}{(2\pi)^3}\kk\sum_{j=\pm}\ab(2j|a_j(\kk)|^2 + ia_j(\kk)a_j(-\kk)e^{-2i\omega(\kk)t} - ia_j^*(\kk)a_j^*(-\kk)e^{2i\omega(\kk)t})> \\
  %                       & = \frac{1}{2\pi c}\int_{\RR^3}\frac{\dl{\kk}}{(2\pi)^3}\kk\ab(|a_+(\kk)|^2 - |a_-(\kk)|^2)
  % \end{align}
\end{proof}
...
\begin{definition}[双方向反射率分布関数 (BRDF)]
  発光 $L_e(x, \omg_r)$
  \begin{align}
